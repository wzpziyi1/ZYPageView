...
class ZYGiftVM: NSObject {
    ...
    lazy var giftlistData : [ZYGiftPackage] = [ZYGiftPackage]()
...
extension ZYGiftVM {
    func loadGiftData(finishedCallback : @escaping () -> ()) {
        ...
        ZYNetworkTool.requestData(.get, urlStr: "http://qf.56.com/pay/v4/giftList.ios", params: ["type" : 0, "page" : 1, "rows" : 150], completedCallback: { result in
            ...
            for i in 0..<dataDict.count {
                ...
                self.giftlistData.append(ZYGiftPackage(dict: dict))
            }
